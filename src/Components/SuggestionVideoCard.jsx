...
function SuggestionVideoCard({ video }) {
  ...
  return <Link to={`/video/${video?.videoId}`}>
    <div className="flex mb-3">
      <div className="relative h-24 lg:h-20 xl:h-24 w-40 min-w-[168px lg-w[32px] lg:min-w-[128px] xl:w-40 xl:min-w-[168px] rounded-xl bg-slate-800 overflow-hidden">
      ...
      <div className="flex flex-col ml-3 overflow-hidden">
        ...
        <span className='text-[12px] lg:text-[10px] xl:text-[12px] font-semibold mt-2 text-white/[0.7] flex items-center'>
          ...
          {video?.author?.badges[0]?.type === "VERIFIED_CHANNEL" && (
            <BsFillCCircleFill className='ml-1 text-[12px] lg:text-[10px] xl:text-[12px] text-white/[0.5]' />
          )}
        </span>
